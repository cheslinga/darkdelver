...
impl BaseMap for Map {
    ...
    fn get_available_exits(&self, idx: usize) -> SmallVec<[(usize, f32); 10]> {
        ...
        if let Some(idx) = self.valid_exit(location, LEFT) { exits.push((idx, 1.0)) }
        if let Some(idx) = self.valid_exit(location, RIGHT) { exits.push((idx, 1.0)) }
        if let Some(idx) = self.valid_exit(location, UP) { exits.push((idx, 1.0)) }
        if let Some(idx) = self.valid_exit(location, DOWN) { exits.push((idx, 1.0)) }

        if let Some(idx) = self.valid_exit(location, UP + LEFT) { exits.push((idx, 1.45)) }
        if let Some(idx) = self.valid_exit(location, DOWN + LEFT) { exits.push((idx, 1.45)) }
        if let Some(idx) = self.valid_exit(location, UP + RIGHT) { exits.push((idx, 1.45)) }
        if let Some(idx) = self.valid_exit(location, DOWN + RIGHT) { exits.push((idx, 1.45)) }
